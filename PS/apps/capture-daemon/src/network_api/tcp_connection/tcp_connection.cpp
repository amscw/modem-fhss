...
std::int64_t tcp_connection::send_data(const char *buffer, std::size_t lenght) const noexcept
{
	assert(buffer != nullptr);
	return send(slave_socket_, buffer, lenght, MSG_NOSIGNAL);
}
...
std::int64_t tcp_connection::receive_data(char *buffer, std::size_t lenght) const noexcept
{
	assert(buffer != nullptr);
	return recv(slave_socket_, buffer, lenght, MSG_NOSIGNAL);
}
