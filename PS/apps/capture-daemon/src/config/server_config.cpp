...
void server_config::get_capture_devname()
{
	try
	{
		cfg_->capture_devname_ = config_["Capture"]["capture_devname"].as<std::string>();
	}
	catch(YAML::ParserException& ex)
	{
		throw ex;
...
void server_config::get_capture_header()
{
	try
	{
		cfg_->capture_header_ = config_["Capture"]["capture_header"].as<uint32_t>();
	}
	catch(YAML::ParserException& ex)
	{
		throw ex;
