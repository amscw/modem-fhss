...
tcp_connection_creator::tcp_connection_creator(std::unique_ptr<endpoint> ep) : tcp_connection_creator()
{
	assert(ep.get());
	ep_ = std::move(ep);
	...
	assert((ep_->get_port() > 0) && (ep_->get_port() < tcp_port_max));
...
{
	if (fcntl(master_socket_, F_SETFL, fcntl(master_socket_, F_GETFL, 0) | O_NONBLOCK) == default_error_code)
	{
...
void tcp_connection_creator::sock_bind() const
{
	...
	{
		throw connection_except(connection_except::error_type::BIND_ERROR);
	}
}
...
void tcp_connection_creator::sock_listen() const
{
	...
	{
		throw connection_except(connection_except::error_type::LISTEN_ERROR);
	}
}
