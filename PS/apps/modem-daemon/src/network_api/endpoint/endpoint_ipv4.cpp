...
endpoint_ipv4::endpoint_ipv4(const std::uint16_t &port)
{
	std::memset(&sock_address4_, 0x00, sizeof(sock_address4_));
	sock_address4_.sin_family = AF_INET;
	sock_address4_.sin_addr.s_addr = htonl(INADDR_ANY);
	sock_address4_.sin_port = htons(port);
}

endpoint_ipv4::endpoint_ipv4(const std::string &ip_address, const std::uint16_t &port)
{
